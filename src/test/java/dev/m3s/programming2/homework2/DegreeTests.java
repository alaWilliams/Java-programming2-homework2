...
public class DegreeTests {
	...
	void myCoursesTests() {
		...
		int numCourses = rand.nextInt(2, 5);
		System.out.println("Initializing a new array called 'newCourses' with " + numCourses + " random StudentCourses:");
		Object[] newCourses = IntStream.range(0, numCourses)
									.mapToObj(i -> randomStudentCourse(false))
									.toArray();
		System.out.println("Adding the contents of the newCourses array to the degree using addStudentCourses method...");
		addStudentCourses.call(degree, newCourses);
		returnedCourses = (Object[]) getCourses.call(degree);
		System.out.println("Each object in the newCourses array should now match the object at the next index in the myCourses array.");
		// getCourses
		for (int i = 0; i < numCourses; i++) {
			// Access the array returned by getCourses
			System.out.println("Comparing values newCourses[" + i + "] and getCourses()[" + (i + 1) + "]");
			assertStudentCoursesEqual(newCourses[i], returnedCourses[i + 1]);
			// Access the array held by myCourses
			System.out.println("Comparing values newCourses[" + i + "] and myCourses[" + (i + 1) + "]");
			assertStudentCoursesEqual(newCourses[i], heldCourses[i + 1]);
		}
	}
