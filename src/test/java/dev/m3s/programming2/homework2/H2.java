...
public class H2 {
	...
	static void assertCoursesEqual(Object course1, Object course2) {
		...
		assertTrue(name.matches(course1, name2), explain(String.format("Expected the name of Course 2 ⟨%s⟩ to match the name of Course 1 ⟨%s⟩", name2, name.get(course1))));

		System.out.print(" => Course 2: ");
		String code2 = courseCode.read(course2);
		System.out.print(" => Course 1: ");
		assertTrue(courseCode.matches(course1, code2), explain(String.format("Expected the courseCode of Course 2 ⟨%s⟩ to match the courseCode of Course 1 ⟨%s⟩", code2, courseCode.get(course1))));

		System.out.print(" => Course 2: ");
		Character base2 = (Character) courseBase.read(course2);
		System.out.print(" => Course 1: ");
		assertTrue(courseBase.matches(course1, base2), explain(String.format("Expected the courseBase of Course 2 ⟨%s⟩ to match the courseBase of Course 1 ⟨%s⟩", base2, courseBase.get(course1))));

		System.out.print(" => Course 2: ");
		int period2 = period.read(course2);
		System.out.print(" => Course 1: ");
		assertTrue(period.matches(course1, period2), explain(String.format("Expected the period of Course 2 ⟨%s⟩ to match the period of Course 1 ⟨%s⟩", period2, period.get(course1))));

		System.out.print(" => Course 2: ");
		double credits2 = credits.read(course2);
		System.out.print(" => Course 1: ");
		assertTrue(credits.matches(course1, credits2), explain(String.format("Expected the credits of Course 2 ⟨%.1f⟩ to match the credits of Course1 ⟨%.1f⟩", credits2, credits.get(course1))));

		System.out.print(" => Course 2: ");
		boolean numericGrade2 = numericGrade.read(course2);
		System.out.print(" => Course 1: ");
		assertTrue(numericGrade.matches(course1, numericGrade2), explain(String.format("Expected the numericGrade of Course 2 ⟨%b⟩ to match the numericGrade of Course 1 ⟨%b⟩", numericGrade2, numericGrade.get(course1))));
	...
	static void assertStudentCoursesEqual(Object studentCourse1,
		...
		System.out.print("=> StudentCourse 1: ");
		assertTrue(gradeNum.matches(studentCourse1, gradeNum2), explain(String.format("Expected the gradeNum of StudentCourse 2 ⟨%d⟩ to match the gradeNum of StudentCourse 1 ⟨%d⟩", gradeNum2, gradeNum.get(studentCourse1))));

		System.out.print("=> StudentCourse 2: ");
		int year2 = yearCompleted.read(studentCourse2);
		System.out.print("=> StudentCourse 1: ");
		assertTrue(yearCompleted.matches(studentCourse1, year2), explain(String.format("Expected the yearCompleted of StudentCourse 2 ⟨%d⟩ to match the yearCompleted of StudentCourse 1 ⟨%d⟩", year2, yearCompleted.get(studentCourse1))));
	...
	static void findIgnoringWhitespace(String expectedPattern, String actual,
		...
		System.out.println("The output should match the regular expression pattern: \"" + shownPattern + "\"");
		System.out.println("Attempting to locate the starting point of the regular expression pattern: \"" + identifyPattern + "\"");
		assertTrue(matcher.find(),
				explain(String.format(
						"%nExpected to be able to find the starting point of the regular expression pattern %n"
								...
								+ "* * * * * *%n*" + (regexExplanation.isBlank()
										...
										: ("%nRegular expression explanation:%n"
												+ regexExplanation)),
						identifyPattern, location, actual)).asIs());
		int originalStartIndex = matcher.start();
		...
		if (!matches){ 
			...
			String packageString = H2.location.toString();
			Pattern objectReferencePattern = Pattern.compile(Pattern.quote(packageString) + "\\.\\w+;@[0-9a-fA-F]+");
	
			Matcher objectReferenceMatcher = objectReferencePattern.matcher(actual); 
	
			// Check for default object string representations. If found, and fail the test without checking error index
					...
					String.format(
							"%nExpected to find a string matching the following regular expression%n"
									...
									+ "* * * * * *%n*" + (regexExplanation.isBlank()
											...
											: ("%nRegular expression explanation:%n"
													+ regexExplanation)),
							shownPattern, location, actual));
			}	
			
			// if the input is valid, but not matching, iterate the output string one char at time in order to find out the error point
			int errorIndex = findErrorIndex(compiled, actualActual);
		
			// Add the original start index to the error index to get the total index in the original string
			errorIndex += originalStartIndex;
			...
			while (startIndex < actual.length()) {
				int spaceIndex = actual.indexOf(" ", startIndex);
				int newlineIndex = actual.indexOf("\n", startIndex);
				int tabIndex = actual.indexOf("\t", startIndex);

				int minIndex = -1;
				if (spaceIndex != -1 && (minIndex == -1 || spaceIndex < minIndex)) {
					minIndex = spaceIndex;
				}
				if (newlineIndex != -1 && (minIndex == -1 || newlineIndex < minIndex)) {
					minIndex = newlineIndex;
				}
				if (tabIndex != -1 && (minIndex == -1 || tabIndex < minIndex)) {
					minIndex = tabIndex;
				}

				if (minIndex != -1) {
					startIndex = minIndex + 1;
					if (minIndex < errorIndex) {
						errorIndex++;
					}
				} else {
					break;
	...
	private static int findErrorIndex(Pattern pattern, String input) {
        ...
        for (currentIndex = 1; currentIndex < input.length()+1; currentIndex++) {
            matcher = matcher.region(0, currentIndex);
            if (matcher.matches() || !matcher.hitEnd()) {
				return currentIndex-1;
            }
        }
		// if the error is not found, it means that the error is due to the string being too short
